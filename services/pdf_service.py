...
# Koordinatlar calibrate_pdf.py çıktısından: python calibrate_pdf.py
# ReportLab Y: sayfa altından yukarıya (0=alt, 841=üst)
def _p1_overlay(c, d):
    sv = lambda k: str(d.get(k) or "")

    # ── PARAGRAF: dönem, tarih/gün ──────────────────────────────────────────
    # calibrate: donem "………" → y=699.0, x=258.6  (Güz/Bahar öncesi)
    #            baslangic "../.../202.." → y=683.1, x=496.4
    #            bitis ".../.../20 2.."  → y=667.3, x=36.0
    #            gun   "……"              → y=667.3, x=173.5
    donem = sv("donem")
    if donem:
        c.setFillColorRGB(1, 1, 1)
        c.rect(255, 694, 38, 11, fill=1, stroke=0)  # "………" placeholder temizle
        _val(c, 259, 700, donem, 8.5)
    bas = sv("baslangic_tarihi")
    bit = sv("bitis_tarihi")
    gun = sv("staj_gun_sayisi")
    if bas:
        c.setFillColorRGB(1, 1, 1)
        c.rect(493, 679, 80, 13, fill=1, stroke=0)   # "../.../202.. ile" alanını temizle
        _val(c, 496, 684, bas + " ile", 8.5)          # tarihin ardına "ile" ekle
    if bit:
        c.setFillColorRGB(1, 1, 1)
        c.rect(33, 663, 52, 13, fill=1, stroke=0)    # x=85'te dur, "tarihleri"(x=88)'yi silme
        _val(c, 36, 668, bit, 8.5)
    if gun:
        c.setFillColorRGB(1, 1, 1)
        c.rect(170, 663, 30, 13, fill=1, stroke=0)   # "……" placeholder (x=173-201)
        _val(c, 173, 668, str(gun), 8.5)
    c.setFillGray(0)

    # ── ÖĞRENCİ ALANLARI ──────────────────────────────────────────────────────
    # calibrate: Bölüm/TC y=611.9  Adı Soyadı y=588.7
    #            Öğrenci/Tel y=565.4  İkametgah y=542.2
    # ":" konumu x=160.3 → veri x=170; TC label x=370.6 → veri x=447
    Y_R1 = 612
    Y_R2 = 589
    Y_R3 = 565
    Y_R4 = 542
    ...
    _val(c, 447, Y_R1, sv("tc_kimlik_no"))
    _val(c, 170, Y_R2, sv("ad_soyad"))
    _val(c, 170, Y_R3, sv("ogrenci_no"))
    _val(c, 398, Y_R3, sv("telefon_no"))
    _wrap_val(c, sv("ikametgah_adresi"), 170, Y_R4, 370, size=8.5)

    # ── STAJ YERİ ALANLARI ────────────────────────────────────────────────────
    # calibrate: Adı y=490.5  Adres y=467.4  Hizmet y=444.2
    #            Tel/Başlama y=420.9  E-posta/Bitiş y=397.7  Web/Fax y=374.6
    # ":" konumu x=157.8 → veri x=165; sağ kolon ":" x=483.6 → veri x=490
    Y_S1 = 491
    Y_S2 = 467
    Y_S3 = 444
    Y_S4 = 421
    ...
    _val(c, 516, Y_S3, sv("haftalik_calisilan_gun"))
    ...
    # calibrate: ÜRETİM x=41.4, İNSAN x=152.7 → checkbox sonraki label öncesi
    #            YÖNETİCİ x=286.9, sayı x=392.8, USTA x=428.1, sayı x=534.1
    DROWS = [
        (306, "d_uretim",    "p_yonetici", "p_usta"),
        (285, "d_pazarlama", "p_muhendis",  "p_teknisyen"),
        (264, "d_muh",       "p_tekniker",  "p_isci"),
        (243, "d_diger1",    "p_diger1",    "p_diger2"),
    ]
    DROWS2 = [
        (306, "d_insan"),
        (285, "d_teknik"),
    ...
    for y_, k1, k3, k4 in DROWS:
        _chk(c, 79,  y_ + 1, bool(d.get(k1)))
        _val(c, 393, y_, sv(k3), 8)
        _val(c, 534, y_, sv(k4), 8)
    for y_, k2 in DROWS2:
        _chk(c, 232, y_ + 1, bool(d.get(k2)))

    # ── İMKANLAR CHECK'LERİ ───────────────────────────────────────────────────
    # calibrate: Ücret x=41.4, Yemek x=144.0, Fotoğraf x=340.9, Nüfus x=452.7
    #            Servis x=41.4, Savcılık x=340.9 → checkbox ~12pt önce
    Y_I1 = 175
    Y_I2 = 161

    _chk(c, 29,  Y_I1 + 1, bool(d.get("ucret")))
    _chk(c, 130, Y_I1 + 1, bool(d.get("yemek")))
    _chk(c, 327, Y_I1 + 1, bool(d.get("fotograf")))
    _chk(c, 439, Y_I1 + 1, bool(d.get("nufus_cuzdani")))
    _chk(c, 29,  Y_I2 + 1, bool(d.get("servis")))
    _chk(c, 327, Y_I2 + 1, bool(d.get("savcilik_belgesi")))
